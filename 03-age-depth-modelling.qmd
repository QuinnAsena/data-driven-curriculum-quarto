...
A more complete version of the modelling process using Bchron can be found in the [vignette](https://cran.r-project.org/web/packages/Bchron/vignettes/Bchron.html). Age-depth modelling is complicated, there are many pitfalls, assumptions, and uncertainties that are often ignored. Recent developments have begun to focus on quantifying uncertainties to understand the reliability of inferences made from the data. In each section we include the key papers on the topic.
