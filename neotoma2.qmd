...
We also filter the ecological groups to *UPHE* (upland herbs) and *TRSH* (trees and shrubs).
More on ecological groups in the [Neotoma Online Manual](https://open.neotomadb.org/manual).
...
Collapsing every *Pinus* morphotype into one taxon is the simplest thing that works, and it is
not what you would do in a real analysis of `r length(unique(allSamp$siteid))` Michigan sites.
The right level to harmonise to depends on the question, on what each analyst recorded, and on
what any dataset you plan to compare against can support.

That last point matters more than it sounds. @sec-harmonisation builds a harmonisation table
for Devil's Lake against a modern pollen database, and finds that the database resolves pine
into hard and soft pines exactly as Neotoma does. Lumping there would have thrown away a
distinction both datasets could see.

This kind of harmonisation is also **within one dataset**, which is the easy case: you only
have to be consistent with yourself. Matching names across two datasets is a different and
much harder problem.
:::
