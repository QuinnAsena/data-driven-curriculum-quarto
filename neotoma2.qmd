...
Two things about that `summarise()` are worth stating plainly, because both are easy to get
wrong. We are summing **counts**, the raw number of grains identified, and summing is the only
correct operation for counts. Had we already converted to proportions, averaging them would be
wrong: a sample counted to 300 grains and one counted to 800 would carry equal weight. Get the
taxonomy settled first, then calculate proportions. That order matters and the book follows it
everywhere.
