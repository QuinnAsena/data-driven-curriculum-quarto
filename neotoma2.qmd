---
author: Simon Goring, Socorro Dominguez, and Quinn Asena
---
...
There are many more functions for manipulating data and conducting other operations that we will encounter along the way while we take you through these four data layers. For this chapter we use several packages, and we can load them all at once with the `pacman` package, which installs anything you do not already have.
...
pacman::p_load(dplyr, neotoma2, geojsonsf, leaflet, DT, stringr)
```
:::

### What the package is actually doing {#sec-apis}

`neotoma2` is a thin wrapper around an application programming interface,
...
You can see the whole mechanism by looking at the address. When you call a function in R like
`get_sites(sitename = "Devils Lake")`, the package assembles the following address and requests it:
...
The samples table contains all the necessary information for plotting data over time and space, for rebuilding chronologies (see @sec-rebuild-chronology), and anything else you need to do. It is also the largest in terms of memory, and so, it is worth saving to disk to avoid repeated `get_downloads()` calls.
...
Before changing anything, look at what is actually there. Devil's Lake and the Michigan sites
both record pine under several names, and it is worth seeing the list before deciding to
collapse it:
...
There are `r sum(stringr::str_detect(neotomatx$variablename, 'Pinus.*'))` of them, and they sit
at different levels: some are species, some are the subgenera *Pinus* and *Strobus*, and some
are simply `Pinus undiff.` where the analyst could not identify further. They also **nest**:
*Pinus strobus* is a species within subg. *Strobus*. That is the problem harmonisation solves.
Across many sites, different analysts identified to different levels, so unless you collapse
them you are comparing a taxon that means something different at each site.

Now the replacement. We use `dplyr` to `mutate()` the column `variablename`, so that any time
we detect (`str_detect()`) a `variablename` starting with `Pinus` (the `.*` is a wildcard for
any character [`.`], zero or more times [`*`]) we `replace()` it with the string `"Pinus"`.
Note this changes *Pinus* in the `allSamp` object; calling `samples()` again would return the
original taxonomy. We also filter the ecological groups to *UPHE* (upland herbs) and *TRSH* (trees and shrubs). More on ecological groups in the [Neotoma Online Manual](https://open.neotomadb.org/manual).
...
Note that in the `summarise()` code, we are *summing* the raw count data (number of individual grains identified). It is important to take care when converting between counts and proportions that manipulations are done in the correct order and proportions are not accidentally summed, or taxa dropped after converting to proportions resulting in taxa not summing to 1.

::: {.callout-note}
# This is a basic demonstration

Collapsing every *Pinus* morphotype into one taxon is the simplest demonstration of programmatically harmonising taxa, and is not necessarily what you want to do in a given dataset, or across all `r length(unique(allSamp$siteid))` Michigan sites. The right level to harmonise to depends on the question, on what each analyst recorded, and often on several ecologically relevant decisions. This kind of harmonisation is also **within one database**, which is the easy case. Matching names across two datasets is a different and much harder problem.

@sec-harmonisation covers decisions we made to build a harmonisation table for Devil's Lake
cross-referenced against the North American Modern Pollen Database (NAMPD).
:::
...
together are `Pinus subg. Pinus` and `Pinus subg. Strobus`. These species/subgenera in
the upper Midwest might have different habitats, fire responses and successional roles.
Look into the ecological roles and functional traits of these species/subgenera and
describe a justifiably approach to aggregating/harmonising taxa.
...
We will explore these data visually, and turn them into pollen diagrams, in @sec-visualisation.
