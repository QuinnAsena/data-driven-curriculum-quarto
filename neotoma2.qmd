...
Note that in the `summarise()` code, we are *summing* the raw count data (number of individual grains identified). It is important to take care when converting between counts and proportions that maniplulations are done in the correct order and proportions are not accidentally summed, or taxa dropped after converting to proportions resulting in taxa not summing to 1.
