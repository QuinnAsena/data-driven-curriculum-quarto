...
```{r summarySitesTable}
#| echo: false
# Workaround, not teaching material. This chunk is echo: false, so the reader never sees
# it. neotoma2 1.0.9's summary() errors on any site whose collection units carry an NA
# handle or an NA datasettype: it na.omit()s the assembled rows, then binds a zero-row
# frame against a length-1 siteid. Two of the 333 Michigan sites (30939 and 30940, both
# named Green Lake) are in that state. See .notes/BUG-neotoma2-summary.md and remove this
# once it is fixed upstream. sites objects reject a logical index, hence which().
summarisable <- which(vapply(mich_sites@sites, function(x) {
  cu <- x@collunits@collunits
  length(cu) > 0 &&
    any(vapply(cu, function(y) !is.na(y@handle) && length(y@datasets) > 0, logical(1)))
}, logical(1)))

neotoma2::summary(mich_sites[summarisable]) %>%
